...
public class D_ArrayImp<T extends Comparable<T>> {
    ...
    public void add(T elem) {
  	...
  	/**
  	 * Deletes element at a specified index and moves back
  	 * all the elements after it,  the array's capacity can
    ...
    //actualización
  	 ...
  	 */
    public void update(int index, T new_elem) {
     ...
     * position) are less than pivot, all elements from index m2 to r are greater than pivot
     * and all the elements between m1 and m2 are equal to pivot
    ...
    private int[] partition3(T[] a, int l, int r) {
      T x = a[l];
      int m1 = l;
      int m2 = l+1;
    ...
    /**
     * Sorts a given list from index l to index r by calling partition3
     * recursively and changing the first element of the range by a random pivot
     * @param a list to sort
     * @param l sorting range start
     * @param r sorting range end
     */
    private void randomizedQuickSort(T[] a, int l, int r) {
        if (l >= r) {
            return;
        }
     ...
     * @return sorted copy of the original array
  	...
  	/**
  	 * Remove's all the elements from this array. The array will be empty after this call
  	 */
    ...
    public boolean inList(T elem) {
  	...
  	/**
  	 * Searchs for a specified element in the array and returns it's index. If the array 
     * doesn't contain such element, -1 is returned
